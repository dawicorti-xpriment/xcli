...
Config.prototype.toObj = function (prefix) {
  ...
  result['OTHER_LDFLAGS'] = list.join(' ');

  if (prefix) {
    _.assign(result, result, function (k) {
      return prefix + k;
    });
  }
...
var extractObj = function (argument, done) {
  if (_.isString(argument) && path.exists(argument)) {
    fs.readFile(argument, 'utf8', function (err, content) {
      if (err) return done(err);

      done(null, objFromFileContent(content));
    });
  } else {
    done(null, argument);
  }
