...
var fs = require('fs');
var otherLinkerFlagsParser = require('./config/otherLinkerFlagsParser');
...
var Config = module.exports = function (obj, throwIt) {
  ...
  this.merge(extractObj(this, obj));
};
...
Config.prototype.toString = function (prefix) {
  ...
  return _.uniq(includeLines.concat(settings)).join('\n');
};
...
Config.prototype.saveAs = function (pathname, prefix) {
  fs.writeFileSync(pathname, this.toString(prefix));
};
...
Config.prototype.toObj = function (prefix) {
  ...
  _.each(['libraries', 'frameworks', 'weak_frameworks'], function (key) {
    ...
    list = _.uniq(list.concat(sorted.map(function (l) {
      return '' + modifier + ' "' + l + '"'; 
    })));
  }, this);
...
var extractObj = function (config, argument) {
  if (_.isString(argument)) {
    return objFromFileContent(config, fs.readFileSync(argument, 'utf8'));
  } else {
    return argument;
  }
};
